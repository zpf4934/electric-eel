...
class WOE:
    ...
    def iv(self, X, y, method='freq', event=1):
        """
        Calculate woe of each feature category and information value
        :param X: 2-D numpy array explanatory features which should be discreted already
        :param y: 1-D numpy array target variable which should be binary
        ...
        self.check_target_binary(y)
        X1 = self.feature_discretion(X, method=method)
        res_iv = []
        woe_dict_list = []
        for i in range(0, X1.shape[-1]):
            x = X1[:, i]
            woe_dict, iv1 = self.woe_single_x(x, y, event)
            res_iv.append(iv1)
            woe_dict_list.append(woe_dict)
        return res_iv, x_col, woe_dict_list
    ...
    def feature_discretion(self, X, method="freq"):
        '''
    ...
    def discrete(self, x):
        '''
        Discrete the input 1-D numpy array using 20 equal percentiles
    ...
    def discrete_distance(self, x):
        ...
        min_v = x.min()
        max_v = x.max()
        size_v = np.round((max_v - min_v) / 10, 2)
        for i in range(10):
            #             point1 = stats.scoreatpercentile(x, i * 10)
            #             point2 = stats.scoreatpercentile(x, (i + 1) * 10)
            point1 = min_v + i * size_v
            point2 = min_v + (i + 1) * size_v
            x1 = x[np.where((x >= point1) & (x <= point2))]
            mask = np.in1d(x, x1)
            res[mask] = (i + 1)
        self.sss = res
